...
Status: ✅ SHIPPED — PR #92 merged (`3657ab4`), `sendWave` deployed to prod (`am---dating-app`, `--project prod`, 2026-07-23 ~10:35). BUG-RADAR-OFF-DISCOVERABLE (P0) closed in code: radar OFF now means invisible + unnotifiable + unmatchable via 4 TDD steps — (1) background presence refresh gated on stored `radar_active` intent; (2) `GeoService.stop()` revocation write retried 3× + Sentry; (3) cold-start reconcile; (4) `sendWave` rejects radar-off targets, client friendly-mapped. Verify was: analyze clean · flutter 507/507 · functions tsc+eslint clean, jest 244/244 · debug APK · full CI green. REMAINING: founder device re-check (Martin scenario). NEXT LANE: `20260723-haptics-stop` (BUG-HAPTICS-PERSIST).
