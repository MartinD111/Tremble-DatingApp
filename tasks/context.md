## Session State — 2026-07-23 (Session 63) — LANE 1 SHIPPED: BUG-RADAR-OFF-DISCOVERABLE MERGED (#92) + `sendWave` DEPLOYED TO PROD

- **✅ MERGED + DEPLOYED.** PR #92 squash-merged to main (`3657ab4`), branch deleted (local + remote). **`firebase deploy --only functions:sendWave --project prod` → `am---dating-app`/europe-west1 "Successful update operation" (2026-07-23 ~10:35 CEST).** Ambient CLI project was `tremble-dev` — deployed with explicit `--project prod` (Rule #101). Client-only changes (background-intent gate, revocation retry, cold-start reconcile, wave error-mapping) ride the next build; only `sendWave` needed a deploy. **Post-merge founder device re-check still owed:** Martin radar-off scenario no longer notifies/matches.
- **Active Task:** `20260723-radar-off-invisible` — first of the five Session-62 fix lanes (order RADAR-OFF ✅ → HAPTICS → WAVEPILL-TAP → ONBOARDING → INTERCEPT). Executed from the ready-to-paste prompt, all 4 steps TDD, one atomic commit each.
- **Environment:** Dev local, branch `fix/radar-off-discoverable` (merged, deleted) off main (build-35 baseline). HIGH-risk lane (Cloud Functions + FCM handler + presence semantics).
- **Delivered (4 commits, all TDD):** (1) `notification_service.dart` — FCM background handler gates the proximity refresh on a new Flutter-prefs `radar_active` intent mirror (written at every toggle funnel + force-reset at cold start), killing the self-sustaining freshness loop (Rule #105). (2) `geo_service.dart` — `stop()` revocation write now retried 3× w/ backoff + `Sentry.captureException` on final failure via new `runRevocationWriteWithRetry`; never throws. (3) `geo_service.dart` + `home_screen.dart` — `reconcileColdStartRadarIntent()` at first auth-ready writes the proximity doc inactive when local intent is off (covers force-kill/FGS death). (4) `matches.functions.ts` — `sendWave` re-verifies target `radarActive:true` + fresh (<2min, same cutoff as scanProximityPairs) else `failed-precondition {reason:'target_radar_off'}`; `wave_repository.dart` maps it to "They're not on the radar right now." via extracted `mapSendWaveException`.
- **Verification (all green):** flutter analyze clean · `flutter test` 507/507 (was 487 baseline; +20 new) · functions tsc + eslint clean · jest 244/244 · debug APK built (`app-dev-debug.apk`).
- **Invariants held:** proximity doc schema unchanged; `scanProximityPairs`/bearing/cooldown untouched; blocked-user eval-without-notify untouched; finder invariants untouched (coords only in `matches/{id}/finder/{uid}`). onWaveCreated NOT modified — sendWave is the sole wave-doc funnel (no client direct-writes to `waves/`), so gating there covers wave-backs; noted as a deliberate scope decision.
- **Open / Next:** lane 2 (BUG-HAPTICS-PERSIST) — handoff prompt in `docs/superpowers/prompts/2026-07-23-session62-fix-prompts.md` PROMPT 2. Then WAVEPILL-TAP → ONBOARDING → INTERCEPT. Founder device checks owed: Martin radar-off re-check (lane 1), iPhone-15 finder re-test (DIAG-IPHONE15-FINDER; toggle hypothesis dead, GNSS-accuracy leading), Android finder pass. Store/legal blockers unchanged (STORE-003/004/005, LEGAL-001/004, CONFIG-REVENUECAT-OFFERINGS).

---

## Session State — 2026-07-23 (Session 62) — BUILD-35 DEVICE PASS: PRECISE FINDER WORKS ON iOS · ROOT-CAUSE AUDIT OF 3 NEW BUGS · FIX PROMPTS WRITTEN

- **🎉 FIRST SUCCESSFUL REAL-WORLD PRECISION FIND (2026-07-22 ~21:00).** iPhone 15 Pro Max: solid arrow + live "NN m" (12/25/39/75 m observed), distance counts down walking, dot tracks phone rotation, movement reflected live. Prod CF logs verify the whole loop: mutual wave → `updateFinderLocation` 3s cadence (both iOS UA + okhttp/Android) → `markMatchFound` (uid `XHXl6Voy…`, 553ms) → block eval-without-notify (`pairsEvaluated:1, pairsNotified:0` while blocked). PR #91 (Podfile.lock sync) merged 2026-07-22 20:37 UTC — nothing open on GitHub.
- **FEATURE-RADAR-SONAR stays OPEN** for: iPhone 15 (non-Pro) re-test (checklist in blockers.md DIAG-IPHONE15-FINDER — check iOS Precise Location toggle FIRST, the >30m-accuracy rejection explains an asymmetric pass), Android/cross-platform pass, and the two spun-out bugs below.
- **AUDIT (this session, all code-traced per Rule #87, NO fixes applied — prompts only):**
  1. **BUG-RADAR-OFF-DISCOVERABLE (P0, HIGH lane):** Martin matched with radar OFF. Chain: `GeoService.stop()` silently swallows the `isActive:false` write (`geo_service.dart:96`) / unclean stop never writes it → FCM background handler refreshes `proximity.updatedAt` on silent pushes with no intent check (`notification_service.dart:24-48`) → self-sustaining freshness loop vs `scanProximityPairs` 2-min cutoff → visible CROSSING_PATHS + wave-able + matchable (no radar check in `sendWave`/`onWaveCreated`). Lesson **Rule #105**.
  2. **BUG-HAPTICS-PERSIST (P1, MEDIUM lane):** vibrations outlive window end + radar off (cross-platform). `currentSearch` (`match_service.dart:22-32`) has no expiry tick (event-driven only → stale expired match forever); `_pingStep` self-recursion's only in-place stopper is `_tickFreshness`'s searching branch, which the precise-finder early-return bypasses (`proximity_ping_controller.dart:160-170`); radar-off toggle stops BLE/service but signals no controller. Lesson **Rule #104**.
  3. **BUG-ONBOARDING-RITUAL-SKIPPED (P2):** `completeOnboarding` auth emit → router redirect `/onboarding`→`/permission-gate` disposes `RegistrationFlow` during the 2500ms hard-lock (`registration_flow.dart:1877-1899`, `router.dart:98-149`) → `mounted` guards silently skip the RitualStep "SIGNAL LOCKED" animation.
  4. **BUG-WAVEPILL-TAP-PROFILE (P2):** pill `onTap` exists but `WavePillService.show` call site (`router.dart:560-571`) never passes it → photo tap does nothing. Want: gated profile card (free→Basic, premium→/profile), no wave side-effect.
  5. **FEATURE-INTERCEPT-RECEIVE-PILL (cluster 3):** receive-side UI for `PULSE_INTERCEPT` pushes is entirely unbuilt; founder spec = side-anchored pop-up pill next to the partner name (photo→left/Send-Photo side, phone→right/Send-Phone side).
  6. Cosmetic: finder fallback shows BOTH "They're close — look around" AND "SEARCHING…" captions stacked (screenshot) — one-caption precedence needed.
- **Docs updated this session:** blockers.md (SESSION 62 section + FEATURE-RADAR-SONAR status), lessons.md (+Rule #104, +Rule #105), this entry. **Ready-to-paste execution prompts: `docs/superpowers/prompts/2026-07-23-session62-fix-prompts.md`** — each lane is planning-free (root cause + files + fix spec + TDD + gates baked in).
- **Founder answers (2026-07-23):** ① b35 AAB **IS on Play Console** (that's how Martin's phone got the build) — question closed. ② iPhone 15 (non-Pro): Precise Location was **ON**, permission **Always** → toggle hypothesis dead. New leading hypothesis in blockers.md DIAG-IPHONE15-FINDER: non-Pro lacks dual-frequency GNSS → real-world accuracy intermittently >30m → caller-side `poor_accuracy` short-circuit (`finder.functions.ts:104-107`) leaves the 15 in fallback while the Pro Max still gets live distance. Diagnostic gap noted: rejection reasons are never logged server-side — add one structured log line in the next finder lane.
- **Open / Next:** run the fix lanes from the prompt file (order: RADAR-OFF → HAPTICS → WAVEPILL-TAP → ONBOARDING-RITUAL → INTERCEPT-RECEIVE), iPhone 15 re-test (revised checklist in blockers.md), Android finder pass. Unchanged: STORE-003/004/005, LEGAL-001/004, CONFIG-REVENUECAT-OFFERINGS, FEATURE-POSTMATCH-NOTIFTAP (folds into WAVEPILL-TAP lane), cluster-2/3 items.
- **Founder roadmap notes (Session 62, not yet lanes):** photo/camera "pole center" fix, map fix, verify gym/run-club/event modes, geofencing for events — park until the above bugs clear.

---

## Session State — 2026-07-22 evening (Session 61 pt. 2) — PR #89 MERGED · BACKEND LIVE · BUILD 35 ON TESTFLIGHT

- **Active Task:** `20260722-precise-finder` release close-out. PR #89 squash-merged to main (`18ff23f`). Branch `chore/build-35` carries: pubspec bump `1.0.0+35` (`ab2381d`), the flicker fix (`5c033a5`), this docs sweep.
- **Pre-release bug hunt (code-reviewer agent on the merged diff) found 1 HIGH, fixed same session:** `updateFinderLocation` wrote `finderOptIn.{uid}=true` unconditionally on every ~3s poll → match-doc snapshot churn → `SonarPingController`/`WarmthController` (both `ref.watch(currentSearchProvider)` in `build()`) re-ran build and reset state → **radar dot blanked every ~1.5–3s during an active finder session.** Fix = compare-before-write (revocation stays unconditional). TDD (3 Jest regressions), 240/240 green, **redeployed to prod**. Full detail: lesson **Rule #102**.
- **PROD DEPLOYS DONE (`am---dating-app`/europe-west1, all with explicit `--project prod`):** ① firestore.rules (finder deny-all) ② `updateFinderLocation` (created, incl. flicker fix) + `markMatchFound` (updated, atomic purge). ③ **Firestore TTL policy on collection group `finder` field `expireAt` = ACTIVE** — gcloud is broken on this Mac (Python 3.9), enabled via Firestore Admin REST API with firebase-tools' stored credentials (lesson **Rule #103**), verified `ttlConfig.state: ACTIVE`.
- **BUILD 35 (`1.0.0+35`) cut via `scripts/release/build_prod.sh all`** (obfuscated, `.env.prod.json` Rule #84, Sentry release `tremble.dating.app@1.0.0+35` finalized, Android symbols uploaded, dSYM `4867C989-941D-3F46-8698-EF9EEE07F68C`). Artifacts preserved to `release-symbols/b35/` (IPA 66M, AAB 64M). **iOS UPLOADED TO TESTFLIGHT — Delivery UUID `7b6785b1-7819-4dd0-8e90-5d159c3adb41`** (first altool attempt died with a Claude session ECONNRESET; retry succeeded — a duplicate upload attempt is a safe way to confirm delivery). **FOUNDER: upload `release-symbols/b35/app-prod-release.aab` to Play Console (versionCode 35).**
- **Payload of build 35** = precise finder (#89) + the C/D/E batch already on main (sweep suppress #85, reveal-avatar loading #87, FGS dataSync drop #88) — build 34 was never cut, so 35 ships it all.
- **Docs swept this session:** blockers.md (FEATURE-RADAR-SONAR → build-35 state + device-pass script), lessons.md (+Rule #102 poll-churn, +Rule #103 gcloud/REST TTL), plan.md (lane status), todo.md, system_map.md (finder data flow), ADR-010 addendum (accepted triangulation risk + implementation deltas).
- **Open / Next:** ⭐ **two-phone device pass on build 35** (script in blockers.md FEATURE-RADAR-SONAR) — closes the feature. Founder: Play AAB upload, merge `chore/build-35` PR. Unchanged: STORE-003/004/005, LEGAL-001/004, CONFIG-REVENUECAT-OFFERINGS, FEATURE-POSTMATCH-NOTIFTAP, cluster-2/3 items.
