## Session State — 2026-07-23 (Session 63) — LANE 1 BUILT: BUG-RADAR-OFF-DISCOVERABLE FIX ON `fix/radar-off-discoverable` (PR PENDING FOUNDER)
...
- **Environment:** Dev local, branch `fix/radar-off-discoverable` off main (build-35 baseline). HIGH-risk lane (Cloud Functions + FCM handler + presence semantics) → founder approval before merge; `sendWave` prod deploy founder-owned (`--project prod`, Rule #101). NOTHING deployed.
...
- **Open / Next:** founder reviews PR (HIGH gate) + runs `firebase deploy --only functions:sendWave --project prod`; post-merge device re-check of the Martin radar-off scenario. Then lane 2 (BUG-HAPTICS-PERSIST). iPhone-15 finder re-test + Android pass still open (see DIAG-IPHONE15-FINDER; toggle hypothesis dead, GNSS-accuracy hypothesis is leading).
