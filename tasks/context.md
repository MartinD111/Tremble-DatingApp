...
- Environment: Dev
- Modified Files:
    - ios/Runner/Info.plist (App Check token updated, remote-notification removed)
    - ios/Runner/Runner.entitlements (aps-environment added/cleared)
    - ios/TrembleRadarWidget/TrembleRadarWidget.entitlements (App Groups removed)
...
- Open Problems: None. Local iOS build completes successfully without codesigning issues.
...
- Completed:
    ...
    - Phase 2 (Native): aps-environment cleared and App Groups disabled to support free Personal Apple Developer accounts. Removed `remote-notification` background mode.
