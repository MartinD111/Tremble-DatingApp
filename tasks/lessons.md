...
**Rule #13 — Server-managed fields (e.g. `lastWaveFoundAt`) MUST be excluded from Client Payloads.**
[2026-04-18] Adding server-managed fields to the `updateProfile` Zod schema creates a security risk (spoofing) and causes API failures if the client sends them. Always exclude internal timestamps, counters, and admin flags from `updateProfileSchema`.
Source: Phase A Audit, April 2024.

**Rule #14 — Normalize User Enums (Gender/Interests) to lowercase in the Data Layer.**
[2026-04-18] The backend Zod schema requires lowercase strings (`male`, `female`). To prevent mismatches between UI strings and API requirements, always use `.toLowerCase()` in the repository's `toApiPayload()` method before sending data to Cloud Functions.
Source: Phase A Audit, April 2024.
