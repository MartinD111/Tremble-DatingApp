...
void main() {
  Future<void> pumpReveal(
    WidgetTester tester, {
    required AsyncValue<MatchProfile?> partnerProfile,
  }) async {
    final match = Match(
      id: 'm1',
      ...
      ProviderScope(
        overrides: [
          ...
          partnerMatchProfileProvider('partner_456')
              .overrideWith((ref) => partnerProfile),
        ],
  ...
  testWidgets('hobby chips render fully above the partner photo (no overlap)',
    ...
    await pumpReveal(tester, partnerProfile: AsyncData(partner));
  ...
  testWidgets('loading partner identity shows spinner instead of bare question',
      (tester) async {
    await pumpReveal(
      tester,
      partnerProfile: const AsyncLoading<MatchProfile?>(),
    );
    ...
    expect(
      find.descendant(
        of: find.byKey(const Key('reveal-avatar')),
  ...
  testWidgets('known partner name without photo falls back to initial',
      (tester) async {
    final partner = MatchProfile(
      id: 'partner_456',
      name: 'Sarah',
